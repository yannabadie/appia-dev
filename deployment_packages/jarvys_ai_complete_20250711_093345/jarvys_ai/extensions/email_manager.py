...
Yann Abadie
    ...
    async def _load_auto_rules(self) -> List[Dict[str, Any]]:
        ...
        return [
            {
                'name': 'urgent_emails',
                'condition': lambda subject: any(word in subject.lower() for word in ['urgent', 'asap', 'important']),
                'action': 'priority_flag',
                'notification': True
            },
            {
                'name': 'meeting_requests',
                'condition': lambda subject: any(word in subject.lower() for word in ['meeting', 'rdv', 'rendez-vous']),
                'action': 'calendar_check',
                'auto_response': True
            },
            {
                'name': 'newsletter_filter',
                'condition': lambda sender: 'newsletter' in sender.lower() or 'no-reply' in sender.lower(),
                'action': 'auto_archive',
                'folder': 'newsletters'
            }
        ]
    ...
    async def _simulate_send_email(self, recipient: str, subject: str, content: str) -> str:
...
Que souhaitez-vous faire exactement ?"""
    ...
    async def _search_emails(self, term: str) -> List[Dict[str, Any]]:
        ...
        for email_data in all_emails:
            if (term.lower() in email_data['subject'].lower() or 
                term.lower() in email_data['sender'].lower() or
                term.lower() in email_data.get('snippet', '').lower()):
