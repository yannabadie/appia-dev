...
class JarvysAITester:
    ...
    async def _test_initialization(self):
        ...
        try:
            ...
            self.test_results['initialization'] = {'status': 'success', 'details': 'JARVYS_AI créé'}
    ...
    async def _test_core_components(self):
        ...
        try:
            ...
            self.test_results['intelligence_core'] = {'status': 'failed', 'error': str(e)}
        ...
        try:
            ...
            self.test_results['continuous_improvement'] = {'status': 'failed', 'error': str(e)}
    ...
    async def _test_extensions(self):
        ...
        for name, extension in self.jarvys.extensions.items():
            try:
                ...
                self.test_results[f'extension_{name}'] = {'status': 'failed', 'error': str(e)}
    ...
    async def _test_commands(self):
        ...
        for command in test_commands:
            try:
                ...
                self.test_results[f'command_{command}'] = {'status': 'failed', 'error': str(e)}
    ...
    async def _test_continuous_improvement(self):
        ...
        try:
            ...
            updates_count = await self.jarvys.continuous_improvement.sync_with_jarvys_dev()
            ...
            self.test_results['continuous_improvement_sync'] = {'status': 'failed', 'error': str(e)}
    ...
    async def _test_fallback_engine(self):
        ...
        try:
            ...
            logger.info(f"✅ Fallback Engine OK (test: {test_result['test_successful']})")
            
        except Exception as e:
            self.test_results['fallback_engine_test'] = {'status': 'failed', 'error': str(e)}
            logger.error(f"❌ Fallback Engine: {e}")
        ...
        if failed_tests > 0:
            ...
            for test_name, result in self.test_results.items():
                if result.get('status') == 'failed':
                    logger.info(f"   - {test_name}: {result.get('error', 'Erreur inconnue')}")
            logger.info("")
