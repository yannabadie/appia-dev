...
class TestAgentWorkflow:
    ...
    def test_agent_workflow_secrets(self):
        ...
        if missing_secrets:
            print(f"Warning: Agent workflow may be missing secrets: {missing_secrets}")
...
class TestJarvysCloudWorkflow:
    ...
    def test_jarvys_cloud_workflow_structure(self):
        ...
        cloud_path = Path(__file__).parent.parent / ".github" / "workflows" / "jarvys-cloud.yml"
    ...
    def test_jarvys_cloud_deployment_steps(self):
        ...
        assert len(found_keywords) > 0, f"Cloud workflow should have deployment steps with keywords: {deployment_keywords}"
...
class TestModelDetectionWorkflow:
    ...
    def test_model_detection_api_keys(self):
        ...
        assert len(found_keys) > 0, f"Model detection should reference at least one API key from: {api_keys}"
...
class TestDashboardDeploymentWorkflow:
    ...
    def test_dashboard_deployment_workflow_exists(self):
        ...
        dashboard_path = Path(__file__).parent.parent / ".github" / "workflows" / "deploy-dashboard.yml"
    ...
    def test_dashboard_supabase_deployment(self):
        ...
        found_keywords = [kw for kw in supabase_keywords if kw.lower() in content.lower()]
        
        assert len(found_keywords) > 0, f"Dashboard deployment should reference Supabase with keywords: {supabase_keywords}"
            ...
            if 'permissions' in workflow_config:
                ...
                for perm in dangerous_permissions:
                    if perm in str(permissions):
                        print(f"Warning: {workflow_file.name} may have excessive permissions: {perm}")
    ...
    def test_workflow_action_versions(self):
        ...
        for workflow_file in workflow_files:
            ...
            if "@main" in content or "@master" in content:
                print(f"Warning: {workflow_file.name} uses unpinned action versions (@main/@master)")
        ...
        for workflow_file in workflow_files:
            ...
            for job_name, job_config in jobs.items():
                if 'timeout-minutes' in job_config:
                    ...
                    if timeout > 60:
                        print(f"Info: {workflow_file.name} job {job_name} has long timeout: {timeout} minutes")
    ...
    def test_workflow_dependency_management(self):
        ...
        for workflow_file in workflow_files:
            ...
            if "poetry" in content.lower():
                assert "poetry install" in content, f"{workflow_file.name} uses Poetry but missing install step"
            
            if "requirements" in content.lower():
                assert "pip install" in content, f"{workflow_file.name} uses requirements but missing install step"
...
class TestWorkflowMonitoring:
    ...
    def test_workflow_logging_practices(self):
        ...
        for workflow_file in workflow_files:
            ...
            found_logging = [indicator for indicator in logging_indicators if indicator in content.lower()]
        ...
        for workflow_file in workflow_files:
            ...
            for job_name, job_config in jobs.items():
                ...
                has_error_handling = any(
                    'continue-on-error' in step or 'if' in step 
                    for step in steps if isinstance(step, dict)
                )
                
                if not has_error_handling and len(steps) > 5:
                    print(f"Info: {workflow_file.name} job {job_name} may lack error handling")