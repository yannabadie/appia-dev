        ...
        try:
            ...
            for cmd in [["docker-compose", "--version"], ["docker", "compose", "--version"]]:
                try:
                    ...
                    if result.returncode == 0:
                        assert "version" in result.stdout.lower(), "Invalid Docker Compose version output"
                        return
                except (subprocess.TimeoutExpired, FileNotFoundError):
                    continue
...
class TestDockerImages:
    ...
    def test_python_base_image_available(self):
        ...
        try:
            result = subprocess.run(
                ["docker", "images", "python:3.12", "--format", "{{.Repository}}:{{.Tag}}"], 
                capture_output=True, 
                text=True, 
                timeout=30
            )
            ...
            if "python:3.12" not in result.stdout:
                try:
                    ...
                    if pull_result.returncode != 0:
                        pytest.skip("Cannot pull Python base image - network may be limited")
    ...
    def test_dockerfile_syntax(self):
        ...
        # Basic syntax checks
        assert any(line.startswith('FROM ') for line in lines), "Dockerfile missing FROM instruction"
        
        # Check for common issues
        for line in lines:
            line = line.strip()
            if line.startswith('FROM '):
                assert not line.endswith(':latest'), "Avoid using :latest tag in Dockerfile"
...
class TestDockerCompose:
    ...
    def test_docker_compose_file_exists(self):
        ...
        assert found_files, f"No Docker Compose files found. Expected one of: {compose_files}"
    ...
    def test_docker_compose_syntax(self):
        ...
        if compose_file.exists():
            try:
                ...
                assert 'services' in config, "Docker Compose file missing 'services' section"
                assert len(config['services']) > 0, "Docker Compose file has no services defined"
...
class TestContainerEnvironment:
    ...
    def test_container_platform_support(self):
        ...
        if system == "Linux":
            ...
            assert machine in ["AMD64", "x86_64"], f"Unsupported Windows architecture: {machine}"
        else:
    ...
    def test_container_resource_limits(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["docker", "run", "--rm", "--memory=100m", "--cpus=0.5", "alpine:latest", "echo", "test"], 
                capture_output=True, 
                text=True, 
                timeout=30
            )
            ...
            if result.returncode != 0:
                if "daemon" in result.stderr.lower():
                    ...
                    pytest.fail(f"Container resource limits test failed: {result.stderr}")
...
class TestContainerBuild:
    ...
    def test_dockerfile_build_context(self):
        ...
        has_copy_or_add = any(line.strip().startswith(('COPY ', 'ADD ')) for line in lines)
    ...
    def test_dockerignore_exists(self):
        ...
        if dockerignore.exists():
            ...
            print("Warning: .dockerignore not found - consider adding for better build performance")
...
class TestProductionReadiness:
    ...
    def test_non_root_user_in_dockerfile(self):
        ...
        if not has_user_setup:
            print("Warning: Dockerfile may be running as root - consider adding non-root user for security")
    ...
    def test_health_check_defined(self):
        ...
        if "healthcheck" not in content:
            print("Info: Consider adding HEALTHCHECK instruction for better monitoring")
    ...
    def test_minimal_image_layers(self):
        ...
        if len(run_commands) > 5:
            print(f"Info: Consider combining RUN commands to reduce layers (found {len(run_commands)})")