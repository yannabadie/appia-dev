...
class JarvysMonitor:
    ...
    def check_system_health(self):
        ...
        missing_secrets = [k for k, v in secrets.items() if not v and k in ['xai_grok', 'claude', 'github', 'supabase']]
        if missing_secrets:
    ...
    def monitor_orchestrator_process(self, process):
        ...
        error_patterns = [
            ...
            "timeout", "DEADLINE_EXCEEDED", "rate limit",
            "authentication", "permission denied", "connection refused"
        ]
        
        warning_patterns = [
            "WARNING", "retry", "fallback", "degraded",
            "slow response", "high latency", "memory usage"
        ]
                ...
                if output:
                    ...
                    for pattern in error_patterns:
                        if pattern.lower() in line_lower:
                            self.log_alert("🔴 CRITICAL", f"Error detected: {pattern}", {"line": line})
                    ...
                    for pattern in warning_patterns:
                        if pattern.lower() in line_lower:
                            self.log_alert("🟡 WARNING", f"Warning detected: {pattern}", {"line": line})
                            break
                    
                    # Track API calls
                    if "grok" in line_lower and ("api" in line_lower or "query" in line_lower):
                        self.stats["api_calls"]["grok"] += 1
                    if "claude" in line_lower and ("api" in line_lower or "validat" in line_lower):
                    ...
                    if "cycle" in line_lower and ("completed" in line_lower or "success" in line_lower):
                        self.stats["cycles_completed"] += 1
                        self.log_alert("🟢 SUCCESS", f"Cycle completed: {self.stats['cycles_completed']}")
    ...
    def health_check_loop(self):
        ...
        while self.monitoring:
            ...
            if health_issues:
                self.log_alert("🔴 CRITICAL", "Health check failed", {"issues": health_issues})
            else:
                self.log_alert("🟢 SUCCESS", "Health check passed")
            
            # Performance check
            runtime = datetime.now() - self.start_time
            if self.stats["cycles_completed"] == 0 and runtime.total_seconds() > 300:  # 5 min no cycles
...
def main():
    ...
    print(f"⏰ End Time: {(datetime.now() + timedelta(minutes=20)).strftime('%H:%M:%S')}")
    ...
    if health_issues:
        monitor.log_alert("🔴 CRITICAL", "Initial health check failed", {"issues": health_issues})
        print("❌ Critical issues detected. Aborting orchestrator launch.")
        return
    else:
        monitor.log_alert("🟢 SUCCESS", "Initial health check passed - All systems operational")
    ...
    try:
        ...
        monitor.log_alert("🟢 SUCCESS", f"Orchestrator launched with PID: {process.pid}")
        ...
        try:
            ...
            monitor.log_alert("🟡 WARNING", "20-minute timeout reached, terminating orchestrator")
            process.terminate()
        ...
        print(f"⏱️ Execution Duration: {report['monitoring_summary']['duration_minutes']} minutes")
        ...
        print(f"🔥 Performance Issues: {report['performance_stats']['performance_issues']}")
        print(f"📞 API Calls - Grok: {report['performance_stats']['api_calls']['grok']}, Claude: {report['performance_stats']['api_calls']['claude']}")
        print(f"🧠 Memory Operations: {report['performance_stats']['memory_operations']}")
