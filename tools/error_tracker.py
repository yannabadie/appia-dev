...
class JarvysErrorTracker:
    ...
    def scan_directory(self, directory: Path, extensions: List[str] = None) -> List[DetectedError]:
        """Scan directory for error patterns."""
        if extensions is None:
            extensions = ['.py', '.log', '.txt', '.yml', '.yaml', '.json', '.md']
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if command == "report":
            ...
            print("Usage: python error_tracker.py [report|json|scan|check] [output_file]")
    else:
        tracker.print_error_summary()
