...
def main():
    ...
    print(f"   ✅ GitHub allow rules: {github_allow_count}/{len(firewall_results)} workflows")
    print(f"   ✅ Copilot allow rules: {copilot_allow_count}/{len(firewall_results)} workflows")
    ...
    no_trailing_count = len([r for r in trigger_results if not r['has_trailing_spaces']])
    valid_yaml_count = len([r for r in trigger_results if r['valid_yaml']])
    
    print(f"   ✅ Quoted 'on:' triggers: {quoted_on_count}/{len(trigger_results)} workflows")
    print(f"   ✅ No trailing spaces: {no_trailing_count}/{len(trigger_results)} workflows")
    print(f"   ✅ Valid YAML syntax: {valid_yaml_count}/{len(trigger_results)} workflows")
    ...
    all_requirements_met = (
        all_have_firewall and
        github_allow_count == len(firewall_results) and
        copilot_allow_count == len(firewall_results) and
        quoted_on_count == len(trigger_results) and
        no_trailing_count == len(trigger_results) and
        valid_yaml_count == len(trigger_results) and
        has_validation and
        has_docs
    )
