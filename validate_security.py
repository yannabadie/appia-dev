...
def validate_repository() -> bool:
    ...
    print(
        f"🔍 Scanning {len(files_to_scan)} files for secrets usage and error handling..."
    )
    ...
    if deprecated_token_files:
        print(
            f"❌ Found {len(deprecated_token_files)} files with deprecated SUPABASE_ACCESS_TOKEN:"
        )
    ...
    if workflow_dir.exists():
        ...
        print(
            f"✅ Found {firewall_configs}/{len(workflow_files)} workflows with firewall configuration"
        )
