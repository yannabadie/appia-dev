...
def fix_create_deployment_package():
    ...
    try:
        ...
        patterns = [
            (r'logger = logging\.getLogger\(__name__\)\.error\(\s*f"([^"]+)"\s*\)', r'logging.getLogger(__name__).error(f"\1")'),
            (r'logger = logging\.getLogger\(__name__\)\.info\("([^"]+)"\)', r'logging.getLogger(__name__).info("\1")'),
            (r'logger = logging\.getLogger\(__name__\)\.info\(\s*f"([^"]+)"\s*\)', r'logging.getLogger(__name__).info(f"\1")'),
        ]
