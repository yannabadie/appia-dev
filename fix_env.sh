#!/bin/bash
# fix_env.sh - Corriger le fichier .env
...
if [ -f .env ]; then
    cp .env .env.bak
    echo "✅ Sauvegarde créée: .env.bak"
fi

# Créer un nouveau .env avec les valeurs par défaut
cat > .env << 'EOF'
# Configuration par défaut - pas de références de variables
...
# Alternative : Créer un script Python qui n'utilise pas dotenv
cat > claude_agent_direct.py << 'EOF'
#!/usr/bin/env python3
"""
...
class ClaudeOpusAgent:
    ...
    def __init__(self):
        ...
        self.github = Github(os.environ.get("GH_TOKEN", os.environ.get("GITHUB_TOKEN", "")))
...
Réponds de manière concise."""
            ...
            with open(f"claude_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md", 'w') as f:
...
chmod +x claude_agent_direct.py

echo -e "\n✅ Correction terminée!"
echo -e "\n🚀 Lancez maintenant:"
echo "poetry run python claude_agent_direct.py"