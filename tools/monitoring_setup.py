...
class JarvysMonitoringSetup:
    ...
    def setup_metrics_database(self) -> bool:
        ...
        try:
            ...
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_perf_timestamp ON performance_benchmarks(timestamp)')
    ...
    def record_metric(self, metric_type: str, metric_name: str, value: float, 
                     metadata: Optional[Dict] = None, component: str = "system") -> bool:
    ...
    def record_health_check(self, component: str, status: str, response_time: float = None,
                           error_message: str = None, details: Dict = None) -> bool:
        ...
        try:
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?)
            ''', (component, status, response_time, error_message, details_json))
    ...
    def run_health_checks(self) -> Dict[str, Any]:
        ...
        if openai_key:
            try:
                ...
                self.record_health_check("openai_api", "healthy", response_time, 
                ...
                self.record_health_check("openai_api", "unhealthy", error_message=str(e)[:100])
        else:
        ...
        if supabase_url and supabase_key:
            try:
                ...
                if "relation" in error_str and "does not exist" in error_str:
                    ...
                    self.record_health_check("supabase_api", "unhealthy", error_message=str(e)[:100])
        else:
    ...
    def cleanup_old_data(self, retention_days: int = 30) -> Dict[str, int]:
        ...
        try:
            ...
            cursor.execute('DELETE FROM performance_benchmarks WHERE timestamp < ?', (cutoff_date,))
    ...
    def get_monitoring_status(self) -> Dict[str, Any]:
        ...
        if self.metrics_db_path.exists():
            status["database"]["size_mb"] = self.metrics_db_path.stat().st_size / (1024 * 1024)
            ...
            try:
                ...
                for table in tables:
                    if table in ['metrics', 'health_checks', 'error_log', 'performance_benchmarks']:
            ...
            while True:
                ...
                if duration_minutes:
                    ...
                    if elapsed_minutes >= duration_minutes:
                        print(f"✅ Monitoring daemon completed {duration_minutes} minute run")
                        break
        ...
        if self.metrics_db_path.exists():
            try:
                ...
                status["recent_metrics"] = [
                    {"type": row[0], "name": row[1], "value": row[2], "timestamp": row[3]}
    ...
    def print_monitoring_summary(self):
        ...
        for category, settings in config.items():
            if isinstance(settings, dict) and 'enabled' in settings:
                ...
                print(f"  {enabled_status} {category.replace('_', ' ').title()}: {settings['enabled']}")
        print()
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if command == "setup":
            ...
            print("Usage: python monitoring_setup.py [setup|status|collect|health|daemon|report|cleanup] [args]")
    else:
        monitor.print_monitoring_summary()
