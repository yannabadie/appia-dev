...
class JarvysHealthChecker:
    ...
    def check_environment_health(self) -> Dict[str, Any]:
        ...
        if sys.version_info.major == 3 and sys.version_info.minor >= 12:
            health["checks"]["python_version"] = {"status": "ok", "value": python_version}
        else:
            health["checks"]["python_version"] = {"status": "warning", "value": python_version}
        ...
        for dir_name in required_dirs:
            ...
            if dir_path.exists():
                ...
                health["checks"][f"dir_{dir_name}"] = {"status": "error", "exists": False}
        ...
        for file_name in config_files:
            ...
            if file_path.exists():
                health["checks"][f"config_{file_name}"] = {"status": "ok", "exists": True}
            else:
                health["checks"][f"config_{file_name}"] = {"status": "warning", "exists": False}
        ...
        if github_token:
            try:
                ...
                if rate_limit.core.remaining < 100:
                    health["warnings"].append(f"GitHub rate limit low: {rate_limit.core.remaining}")
                    if health["status"] == "healthy":
                        health["status"] = "warning"
    ...
    def check_services_health(self) -> Dict[str, Any]:
        ...
        if metrics_db.exists():
            try:
                ...
                if age_hours > 24:
                    health["warnings"].append(f"Metrics database is {age_hours:.1f} hours old")
                    if health["status"] == "healthy":
                        health["status"] = "warning"
        ...
        for workflow_name in required_workflows:
            ...
            if workflow_path.exists():
                try:
                    ...
                    health["errors"].append(f"Workflow {workflow_name} has syntax errors")
                    health["status"] = "unhealthy"
            else:
                health["checks"][f"workflow_{workflow_name}"] = {
                    "status": "warning",
                ...
                health["warnings"].append(f"Required workflow {workflow_name} not found")
                if health["status"] == "healthy":
                    health["status"] = "warning"
    ...
    def run_comprehensive_health_check(self) -> Dict[str, Any]:
        ...
        for component_name, check_function in components.items():
            ...
            # Count checks
            overall_health["summary"]["total_checks"] += len(component_health.get("checks", {}))
            ...
            elif component_health["status"] == "warning" and overall_health["overall_status"] != "unhealthy":
    ...
    def print_health_summary(self):
        ...
        for component_name, component_health in health['components'].items():
            ...
            print(f"  {status_emoji} {component_name.title()}: {component_health['status']}")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if command == "report":
            ...
            print("Usage: python health_check.py [report|json|summary|check] [output_file]")
    else:
        checker.print_health_summary()
