...
class TestSupabaseDeployment:
    ...
    def test_supabase_functions_deployment(self):
        ...
        for func_dir in function_dirs:
            ...
            if index_file.exists():
                ...
                assert any(pattern in content for pattern in ["Deno.serve", "new Response"]), \
    ...
    def test_supabase_deployment_script(self):
        ...
        if deploy_script.exists():
            ...
            assert "supabase" in content.lower(), "Deploy script should use Supabase CLI"
            
            # Should be executable
            assert deploy_script.stat().st_mode & 0o111, "Deploy script should be executable"
        else:
    ...
    def test_supabase_cli_available(self):
        ...
        try:
            ...
            if result.returncode == 0:
                assert "supabase" in result.stdout.lower(), "Should return Supabase version"
            else:
...
class TestDockerDeployment:
    ...
    def test_dockerfile_deployment_ready(self):
        ...
        assert any(line.startswith('FROM ') for line in lines), "Should have FROM instruction"
        assert any(line.startswith('WORKDIR ') for line in lines), "Should set working directory"
    ...
    def test_dockerignore_exists(self):
        ...
        if dockerignore.exists():
            ...
            recommended_ignores = [".git", "__pycache__", "*.pyc", ".pytest_cache"]
            found_ignores = [ignore for ignore in recommended_ignores if ignore in content]
            
            assert len(found_ignores) > 0, f"Should ignore some common files: {recommended_ignores}"
        else:
            print("Info: .dockerignore not found - consider adding for better build performance")
...
class TestGitHubPagesDeployment:
    ...
    def test_pages_workflow_exists(self):
        ...
        for workflow_file in workflows_path.glob("*.yml"):
            ...
            if any(keyword in content.lower() for keyword in ['pages', 'github-pages', 'docs']):
    ...
    def test_mkdocs_deployment_ready(self):
        ...
        if mkdocs_config.exists():
            ...
            assert 'docs_dir' in config or 'docs' in str(config), "Should specify docs directory"
...
class TestDeploymentErrorHandling:
    ...
    def test_deployment_rollback_capability(self):
        ...
        for workflow_file in workflows_path.glob("*.yml"):
            ...
            found_rollback = [pattern for pattern in rollback_patterns if pattern in content.lower()]
            if found_rollback:
    ...
    def test_deployment_health_checks(self):
        ...
        for workflow_file in workflows_path.glob("*.yml"):
            ...
            if any(keyword in workflow_file.name.lower() for keyword in ['deploy', 'build']):
                found_health = [pattern for pattern in health_patterns if pattern in content.lower()]
                if found_health:
    ...
    def test_deployment_notification_on_failure(self):
        ...
        for workflow_file in workflows_path.glob("*.yml"):
            ...
            if any(keyword in workflow_file.name.lower() for keyword in ['deploy', 'build']):
                found_notifications = [pattern for pattern in notification_patterns if pattern in content.lower()]
                if found_notifications:
                    print(f"Notifications in {workflow_file.name}: {found_notifications}")
                ...
                for step in steps:
                    if isinstance(step, dict) and 'uses' in step:
                        if 'retry' in step.get('with', {}):
                            print(f"Retry logic found in {workflow_file.name}, step: {step.get('name', 'unnamed')}")
...
class TestDeploymentSecurity:
    ...
    def test_deployment_secrets_protection(self):
        ...
        for workflow_file in workflows_path.glob("*.yml"):
            ...
            assert not found_secrets, f"Hardcoded secrets found in {workflow_file.name}: {found_secrets}"
        ...
        for workflow_file in workflows_path.glob("*.yml"):
            ...
            for job_name, job_config in jobs.items():
                if 'environment' in job_config:
                    ...
                    print(f"Environment protection in {workflow_file.name}: {environment}")
        ...
        for workflow_file in workflows_path.glob("*.yml"):
            ...
            if 'permissions' in workflow_config:
                ...
                if permissions == 'write-all' or permissions == {}:
                    print(f"Warning: {workflow_file.name} may have excessive permissions")
                else:
                    print(f"Permission restrictions in {workflow_file.name}: {permissions}")
...
class TestDeploymentMonitoring:
    ...
    def test_deployment_logging(self):
        ...
        for workflow_file in workflows_path.glob("*.yml"):
            ...
            if any(keyword in workflow_file.name.lower() for keyword in ['deploy', 'build']):
                ...
                found_logging = [pattern for pattern in logging_patterns if pattern in content.lower()]
    ...
    def test_deployment_metrics_collection(self):
        ...
        metrics_files = list(project_root.glob("*metric*")) + list(project_root.glob("*monitor*"))
    ...
    def test_deployment_status_reporting(self):
        ...
        for workflow_file in workflows_path.glob("*.yml"):
            ...
            for job_name, job_config in jobs.items():
                ...
                status_steps = [step for step in steps if isinstance(step, dict) 
                              and any(keyword in str(step).lower() 
                                    for keyword in ['status', 'report', 'notify'])]
                
                if status_steps:
                    print(f"Status reporting in {workflow_file.name}: {len(status_steps)} steps")
...
class TestDeploymentPerformance:
    ...
    def test_deployment_caching(self):
        ...
        for workflow_file in workflows_path.glob("*.yml"):
            ...
            found_caching = [pattern for pattern in cache_patterns if pattern in content.lower()]
    ...
    def test_deployment_parallelization(self):
        ...
        for workflow_file in workflows_path.glob("*.yml"):
            ...
            if len(jobs) > 1:
                ...
                if parallel_jobs > 1:
                    print(f"Parallel execution in {workflow_file.name}: {parallel_jobs} parallel jobs")
        ...
        for workflow_file in workflows_path.glob("*.yml"):
            ...
            for job_name, job_config in jobs.items():
                ...
                if 'timeout-minutes' in job_config:
                    ...
                    print(f"Resource timeout in {workflow_file.name}: {timeout} minutes")
                
                # Check for runner selection
                if 'runs-on' in job_config:
                    runner = job_config['runs-on']
                    if 'ubuntu-latest' in str(runner):
                        print(f"Using optimized runner in {workflow_file.name}: {runner}")