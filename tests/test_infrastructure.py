...
class TestSupabaseInfrastructure:
    ...
    def test_supabase_schema_sql_exists(self):
        ...
        assert "CREATE" in content.upper(), "schema.sql should contain CREATE statements"
        ...
        if function_dirs:
            ...
            for func_dir in function_dirs:
                ...
                if index_file.exists():
                    ...
                    assert "Deno.serve" in content or "new Response" in content, \
                        f"Function {func_dir.name} doesn't look like a valid Edge Function"
...
class TestGCPInfrastructure:
    ...
    def test_gcp_service_account_permissions(self):
        ...
        try:
            ...
            assert ".iam.gserviceaccount.com" in client_email, \
...
class TestDatabaseSchema:
    ...
    def test_schema_sql_syntax(self):
        ...
        assert content.count('(') == content.count(')'), "Unmatched parentheses in schema.sql"
        assert content.count('{') == content.count('}'), "Unmatched braces in schema.sql"
    ...
    def test_functions_sql_exists(self):
        ...
        if functions_path.exists():
            ...
            assert "CREATE" in content.upper(), "functions.sql should contain CREATE statements"
...
class TestStorageConfiguration:
    ...
    def test_memory_database_file_location(self):
        ...
        if memory_db.exists():
            ...
            assert memory_db.stat().st_size >= 0, "Memory database file should be readable"
        else:
        ...
        for func_dir in function_dirs:
            ...
            if index_file.exists():
                ...
                assert any(pattern in content for pattern in ["Deno.serve", "new Response"]), \
                    f"Function {func_dir.name} doesn't contain proper Edge Function patterns"
    ...
    def test_edge_function_deployment_ready(self):
        ...
        for func_dir in function_dirs:
            ...
            if index_file.exists():
                ...
                has_cors = "cors" in content.lower() or "Access-Control-Allow" in content
                has_error_handling = "try" in content and "catch" in content
                
                if not has_cors:
                    print(f"Warning: Function {func_dir.name} may need CORS handling")
                if not has_error_handling:
                    print(f"Warning: Function {func_dir.name} may need error handling")