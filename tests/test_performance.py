        ...
        for i in range(iterations):
            try:
                ...
                measurement = {
                    ...
                    "result": result if isinstance(result, (int, float, str, bool, type(None))) else str(result)
                }
...
class JarvysPerformanceTester:
    ...
    def test_api_client_creation(self) -> bool:
        ...
        try:
            ...
            supabase_client = create_client("https://test.supabase.co", "test-key")
    ...
    def benchmark_component(self, component_name: str, test_func: Callable, 
                          load_levels: List[int] = None) -> Dict[str, Any]:
    ...
    def print_performance_summary(self):
        ...
        for test_name, test_result in results['test_results'].items():
            if test_result['status'] == 'success' and 'statistics' in test_result:
                ...
                status_icon = "✅" if mean_time < 1.0 else "⚠️" if mean_time < 5.0 else "❌"
                print(f"  {status_icon} {test_name.replace('_', ' ').title()}: {mean_time:.3f}s avg")
                
                if mean_time > 1.0:
                    print(f"    Range: {duration_stats.get('min', 0):.3f}s - {duration_stats.get('max', 0):.3f}s")
            else:
                print(f"  ❌ {test_name.replace('_', ' ').title()}: {test_result['status']}")
        ...
        slow_tests = [
            ...
            result.get('statistics', {}).get('duration', {}).get('mean', 0) > 1.0
        ]
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if command == "report":
            ...
            print("Usage: python test_performance.py [report|json|test|run] [output_file]")
    else:
        tester.print_performance_summary()
