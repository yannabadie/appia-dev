...
class JarvysLogAnalyzer:
    ...
    def _analyze_log_line(self, line: str, analysis: Dict[str, Any]):
        ...
        # Count log levels
        if any(pattern in line_lower for pattern in ['error', 'err', 'failed', 'exception']):
        ...
        # Look for API calls
        if any(pattern in line_lower for pattern in ['http', 'api', 'request', 'response']):
        ...
        common_words = {
            ...
            'this', 'that', 'these', 'those', 'it', 'they', 'them', 'their', 'there'
        }
    ...
    def analyze_all_logs(self) -> Dict[str, Any]:
        ...
        for log_file in log_files:
            ...
            overall_analysis["files"][str(log_file.relative_to(self.project_root))] = file_analysis
            
            # Update summary
            file_info = file_analysis["file_info"]
            content_analysis = file_analysis["content_analysis"]
            
            overall_analysis["summary"]["total_size_mb"] += file_info["size_bytes"] / (1024 * 1024)
            overall_analysis["summary"]["total_errors"] += content_analysis["error_count"]
            overall_analysis["summary"]["total_warnings"] += content_analysis["warning_count"]
            overall_analysis["summary"]["total_lines"] += content_analysis["total_lines"]
            
            # Track oldest/newest
            file_age = file_info["age_hours"]
            if overall_analysis["summary"]["oldest_log"] is None or file_age > overall_analysis["summary"]["oldest_log"]:
                overall_analysis["summary"]["oldest_log"] = file_age
            if overall_analysis["summary"]["newest_log"] is None or file_age < overall_analysis["summary"]["newest_log"]:
            ...
            for error in file_analysis["patterns"]["errors"]:
                ...
                overall_analysis["aggregated_patterns"]["common_errors"][error_type] += 1
            
            for warning in file_analysis["patterns"]["warnings"]:
                warning_type = self._extract_error_type(warning)
                overall_analysis["aggregated_patterns"]["common_warnings"][warning_type] += 1
            
            # Aggregate keywords
            overall_analysis["aggregated_patterns"]["frequent_keywords"].update(file_analysis["keywords"])
        
        # Round total size
        overall_analysis["summary"]["total_size_mb"] = round(overall_analysis["summary"]["total_size_mb"], 2)
    ...
    def print_log_summary(self):
        ...
        if analysis['aggregated_patterns']['common_errors']:
            ...
            for error_type, count in list(analysis['aggregated_patterns']['common_errors'].items())[:5]:
        ...
        if analysis['aggregated_patterns']['common_warnings']:
            ...
            for warning_type, count in list(analysis['aggregated_patterns']['common_warnings'].items())[:5]:
        ...
        if analysis['aggregated_patterns']['frequent_keywords']:
            ...
            keywords = list(analysis['aggregated_patterns']['frequent_keywords'].items())[:10]
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if command == "report":
            ...
            print("Usage: python log_analyzer.py [report|json|summary|analyze] [output_file]")
    else:
        analyzer.print_log_summary()
