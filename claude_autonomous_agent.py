...
class ClaudeOpusAgent:
    ...
    def __init__(self):
        ...
        self.cost_limit_daily = 20.0  # $20 par jour
    ...
    async def scan_code_issues(self, repo_name: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for issue in result.get("issues", []):
                issues.append({"type": "code_issue", "repo": repo_name, **issue})
    ...
    async def execute_task(self, task: Dict[str, Any]):
        ...
        logger.info(
            f"🔧 Exécution de la tâche: {task.get('title', task.get('description'))}"
        )
    ...
    async def fix_code_issue(self, issue: Dict[str, Any]):
        ...
        try:
            ...
            logger.error(f"Impossible de lire {file_path}")
            return
        ...
        branch_name = (
            f"claude-fix-{issue['type']}-{datetime.now().strftime('%Y%m%d%H%M%S')}"
        )
...
"""
            ...
            try:
                ...
                self.supabase.table("pending_fixes").update(
                    {"pr_created": True, "pr_number": pr.number}
                ).eq("id", fix["id"]).execute()

                logger.info(f"✅ PR créée: #{pr.number}")

            except Exception as e:
                logger.error(f"Erreur création PR: {e}")
    ...
    async def get_completed_tasks_count(self) -> int:
        ...
        result = (
            self.supabase.table("pending_fixes")
            .select("*")
    ...
    async def analyze_performance(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_tasks_per_run": (
                sum(log.get("tasks_completed", 0) for log in logs.data) / total_runs
                if total_runs > 0
                else 0
            ),
        ...
        async def handle_connection(websocket, path):
            async for message in websocket:
                ...
                if data["command"] == "fix_current_file":
                    result = await self.fix_current_file(
                        data["content"], data["filepath"]
                    )
...
async def main():
    ...
    await asyncio.gather(agent.autonomous_loop(), vscode_ext.start_server())
